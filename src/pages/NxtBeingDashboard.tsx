...
const getStatsConfig = (dashboardData: any) => {
  return [
    { 
      title: "Profile Views", 
...
export default function NxtBeingDashboard() {
  ...
  // Load dashboard data from database on component mount
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const data = await apiService.getApplicantDashboard(user.id);
  ...
  const statsConfig = getStatsConfig(dashboardData);

  // Use real data from database or show loading/error states
  const displayName = dashboardData?.profile?.name || user?.firstName || 'User';
  const displayStats = dashboardData?.stats || {};
  const displayJobs = dashboardData?.recommendedJobs || [];
  const displaySkills = dashboardData?.skills?.ai || [];
  const displayActivity = dashboardData?.recentActivity || [];
        ...
        >
          <GlassCard>
            <div className="flex items-center justify-between mb-6">
              ...
              <a href="/nxtbeing/jobs" className="text-primary-500 hover:text-primary-400 text-sm font-medium">
                View all
              </a>
            </div>
            <div className="space-y-4">
              {displayJobs.length > 0 ? (
                displayJobs.map((job, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {job.skills.slice(3, 6).map((skill: string) => (
                        <span
                          key={skill}
                ...
                <div className="text-center py-8">
                  <p className="text-white/60">No recommended jobs available yet.</p>
                  <p className="text-white/40 text-sm mt-2">Complete your profile to get personalized job recommendations.</p>
                </div>
              )}
            </div>
          </GlassCard>
        ...
        >
          ...
          <GlassCard>
            ...
            <div className="space-y-4">
              {displaySkills.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="w-full bg-white/10 rounded-full h-2 glass-glow">
                      <div
                        ...
                        style={{ width: `${(skill.yearsOfExperience / 5) * 100}%` }}
                      />
                    </div>
                    <div className="text-right">
                      <span className="text-lg font-bold text-primary-400 tracking-wide futuristic-glow">
                        {Math.round((skill.yearsOfExperience / 5) * 100)}%
                      </span>
                    </div>
                  </motion.div>
                ))
                ...
                <div className="text-center py-8">
                  <p className="text-white/60">No AI skills recorded yet.</p>
                  <p className="text-white/40 text-sm mt-2">Add your AI skills to see your progress.</p>
                </div>
              )}
            </div>
          </GlassCard>
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-6">
              ...
              <a href="/nxtbeing/applications" className="text-primary-500 hover:text-primary-400 text-sm font-medium">
                View all
              </a>
            </div>
            <div className="space-y-4">
              {displayActivity.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-white/60">{new Date(activity.timestamp).toLocaleDateString()}</p>
                  </div>
                </motion.div>
              ))
              ) : (
                <div className="text-center py-8">
                  ...
                  <p className="text-white/40 text-sm mt-2">Your activity will appear here.</p>
                </div>
              )}
            </div>
          </GlassCard>
